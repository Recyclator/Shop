...
async function loadRoles(){try{const r=await fetch(`${getBaseApiUrl()}/roles`,{headers:{'Authorization':'Bearer '+getAuthToken()}});if(r.ok){rpRoles=(await r.json()).data||[];renderRoles();}}catch(e){}}
...
async function loadPermissions(){try{const r=await fetch(`${getBaseApiUrl()}/permissions`,{headers:{'Authorization':'Bearer '+getAuthToken()}});if(r.ok){rpPerms=(await r.json()).data||[];renderAllPerms();}}catch(e){}}
...
async function saveRole(){
    ...
    try{const res=await fetch(id?`${getBaseApiUrl()}/roles/${id}`:`${getBaseApiUrl()}/roles`,{method:id?'PUT':'POST',headers:{'Authorization':'Bearer '+getAuthToken(),'Content-Type':'application/json'},body:JSON.stringify(b)});const d=await res.json();if(res.ok){Toast.success('Éxito',id?'Rol actualizado':'Rol creado');closeRoleModal();loadRoles();}else{Toast.error('Error',d.error?.message||'Error');}}catch(e){Toast.error('Error','Error de conexión');}
}

async function deleteRole(id){if(!confirm('¿Eliminar este rol?'))return;try{const res=await fetch(`${getBaseApiUrl()}/roles/${id}`,{method:'DELETE',headers:{'Authorization':'Bearer '+getAuthToken()}});if(res.ok){Toast.success('Éxito','Rol eliminado');loadRoles();}else{Toast.error('Error',(await res.json()).error?.message||'Error');}}catch(e){}}
